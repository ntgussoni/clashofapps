...
import { type AppInfo, type Review, type UnifiedAppInfo, type UnifiedReview, type Platform } from "@/types";
import { fetchAppStoreData, type AppStoreAppInfo, type AppStoreReview } from "./appStoreDataFetcher";
...
function normalizeGooglePlayApp(appInfo: AppInfo): UnifiedAppInfo {
  return {
    ...
    categories: appInfo.categories as Array<{ name: string; id: string | null }>,
...
export async function fetchAppData(
  ...
  if (detectedPlatform === "APP_STORE") {
    const { appInfo, reviews } = await fetchAppStoreData(appStoreId, reviewCount);
    return {
      appInfo: normalizeAppStoreApp(appInfo),
      reviews: reviews.map(normalizeAppStoreReview),
    };
  } else {
    const { appInfo, reviews } = await fetchGooglePlayData(appStoreId, reviewCount);
    return {
